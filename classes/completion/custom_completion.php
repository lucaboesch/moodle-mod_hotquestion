...
class custom_completion extends activity_custom_completion {
    ...
    public function get_state(string $rule): int {
        ...
        if ($rule === 'completionpost') {
            if (empty($hotquestion->completionpost)) {
                return COMPLETION_INCOMPLETE;
            }

            $count = $DB->count_records('hotquestion_questions', [
                'hotquestion' => $hotquestionid,
                'userid' => $userid,
            ]);
            return ($count >= (int)$hotquestion->completionpost) ? COMPLETION_COMPLETE : COMPLETION_INCOMPLETE;
        }

        if ($rule === 'completionvote') {
            if (empty($hotquestion->completionvote)) {
                return COMPLETION_INCOMPLETE;
            }

            $sql = "SELECT COUNT(hv.id)
                      FROM {hotquestion_votes} hv
                      JOIN {hotquestion_questions} hqq ON hqq.id = hv.question
                     WHERE hqq.hotquestion = :hotquestionid
                       AND hv.voter = :userid";
            $params = ['hotquestionid' => $hotquestionid, 'userid' => $userid];
            $count = (int)$DB->count_records_sql($sql, $params);
            return ($count >= (int)$hotquestion->completionvote) ? COMPLETION_COMPLETE : COMPLETION_INCOMPLETE;
        }

        if ($rule === 'completionpass') {
            if (empty($hotquestion->completionpass) || empty($hotquestion->grade)) {
                return COMPLETION_INCOMPLETE;
            }
            ...
            return $grades[$userid]->is_passed($item) ? COMPLETION_COMPLETE : COMPLETION_INCOMPLETE;
        }
    ...
    public function get_custom_rule_descriptions(): array {
        ...
        $gradeenabled = !empty($this->cm->customdata['gradeenabled']);
        return [
            ...
            'completionpass' => ($gradeenabled && !empty($completionpass))
                ? get_string('completiondetail:pass', 'hotquestion', $completionpass)
                : '',
        ];
    }
